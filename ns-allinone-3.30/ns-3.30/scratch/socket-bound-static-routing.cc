...
main (int argc, char *argv[])
{
  ...
  NodeContainer c = NodeContainer (nSrc, nDst, nRtr1, nRtr2, nDstRtr);   // create a nodecontainer for all nodes

  InternetStackHelper internet;
  internet.Install (c);              // Install internet stack for all nodes
  ...
  PointToPointHelper p2p;                                             // create a p2p channel
  p2p.SetDeviceAttribute ("DataRate", StringValue ("5Mbps"));
  p2p.SetChannelAttribute ("Delay", StringValue ("2ms"));
  NetDeviceContainer dSrcdRtr1 = p2p.Install (nSrcnRtr1);            // install p2p on the nodes
  ...
  Ptr<NetDevice> SrcToRtr1=dSrcdRtr1.Get (0);           // N3ns33PtrINS_9NetDeviceEEE
